...
export async function runOrDebugAllFeaturesInOneInstance(
  wr: WkspRun
): Promise<void> {
...
export async function runOrDebugFeatures(
  wr: WkspRun,
  parallelMode: boolean,
  scenarioQueueItems: QueueItem[]
): Promise<void> {
  ...
  try {
    ...
    const pipedPathPatterns = getPipedFeaturePathsPattern(
      wr,
      parallelMode,
      scenarioQueueItems
    );
...
export async function runOrDebugFeatureWithSelectedScenarios(
  wr: WkspRun,
  parallelMode: boolean,
  selectedScenarioQueueItems: QueueItem[]
): Promise<void> {
  ...
  try {
    ...
    const pipedScenarioNames = getPipedScenarioNames(
      selectedScenarioQueueItems
    );
    const friendlyEnvVars = getFriendlyEnvVars(wr.wkspSettings);
    const { ps1, ps2 } = getPSCmdModifyIfWindows();
    const featureFileWorkspaceRelativePath =
      selectedScenarioQueueItems[0].scenario.featureFileWorkspaceRelativePath;
...
function getPipedFeaturePathsPattern(
  wr: WkspRun,
  parallelMode: boolean,
  filteredChildItems: QueueItem[]
) {
  ...
  if (!parallelMode) {
    ...
    const selectedFolderIds =
      wr.request.include?.filter((x) => !x.uri).map((x) => x.id) ?? [];

    folderPaths.push(
      ...selectedFolderIds.map(
        (id) =>
          vscode.workspace.asRelativePath(vscode.Uri.parse(id), false) + "/"
      )
    );
  ...
  const distinctFeaturePaths = [
    ...new Set(
      filteredChildItems.map(
        (qi) => qi.scenario.featureFileWorkspaceRelativePath
      )
    ),
  ];

  // remove any feature path already covered by a parent folder selected by the user
  const featurePathsNotCoveredByFolderPaths = distinctFeaturePaths.filter((x) =>
    folderPaths.every((y) => !x.includes(y))
  );
...
function getPipedScenarioNames(selectedScenarios: QueueItem[]) {
  ...
  selectedScenarios.forEach((x) => {
    scenarioNames.push(
      getScenarioRunName(x.scenario.scenarioName, x.scenario.isOutline)
    );
  });
