...
export type parseJunitFileResult = {
  junitContents: JunitContents;
  fsPath: string;
};
...
export function updateTest(
  ...
  switch (result.status) {
    ...
    //       `JUnit result was "untested". Check output in ${window}.`
    //     )
    //   );
    //   break;
    case "failed":
      if (!item.test.uri || !item.test.range) throw "invalid test item";
...
function CreateParseResult(
  ...
  if (actualDuration) xmlDuration = actualDuration;
  ...
  if (xmlStatus === "untested") {
    ...
    else config.logger.show(wkspSettings.uri);
    return { status: "untested", duration: xmlDuration };
  }

  if (xmlStatus !== "failed") {
    throw new Error(
      `Unrecognised behave scenario status result "${xmlStatus}" found while parsing junit file ` +
  ...
  const concatErrText = (testCase: TestCase) => {
    const build = (reasons: Reason[]) => {
      ...
      reasons.forEach((reason) => {
        ...
        if (reason._) reasonBlock += reason._.trim();
  ...
  reasonBlocks.forEach((reason) => {
    ...
    lines.forEach((line) => {
      if (!line.startsWith("Location: ") && /None$/.exec(line) === null) {
        errText +=
          line
            .replace(/ ... failed in .+\..+s$/, " ... failed")
            .replace(/ ... undefined in .+\..+s$/, " ... undefined") + "\n";
      }
    });
  });
...
function getjUnitName(
  wkspSettings: WorkspaceSettings,
  featureFileName: string,
  featureFileWorkspaceRelativePath: string
) {
  const featureFileStem = featureFileName.replace(/.feature$/, "");

  // default
  let dotSubFolders = featureFileWorkspaceRelativePath
    .replace(wkspSettings.workspaceRelativeFeaturesPath + "/", "")
    .split("/")
    .slice(0, -1)
    .join(".");
  ...
  if (
    !wkspSettings.stepsSearchUri.path.startsWith(wkspSettings.featuresUri.path)
  ) {
    if (featureFileWorkspaceRelativePath === "features/" + featureFileName) {
      ...
      if (os.platform() === "win32") {
        const lastDir = wkspSettings.workspaceRelativeFeaturesPath
          .split("/")
          .pop();
        if (lastDir === "features")
          dotSubFolders = dotSubFolders
            ? lastDir + "." + dotSubFolders
            : lastDir;
      }
    }
  }
...
function getJunitFileUri(
  wkspSettings: WorkspaceSettings,
  queueItem: QueueItem,
  wkspJunitRunDirUri: vscode.Uri
): vscode.Uri {
  const junitName = getjUnitName(
    wkspSettings,
    queueItem.scenario.featureFileName,
  ...
  if (os.platform() !== "win32") return junitFileUri;

  if (junitFileUri.fsPath.length <= WIN_MAX_PATH) return junitFileUri;
...
export function getWkspQueueJunitFileMap(
  wkspSettings: WorkspaceSettings,
  run: vscode.TestRun,
  wkspQueueItems: QueueItem[]
) {
...
export async function parseJunitFileAndUpdateTestResults(
  wkspSettings: WorkspaceSettings,
  run: vscode.TestRun,
  debug: boolean,
  junitFileUri: vscode.Uri,
  ...
  try {
    ...
    updateTestResultsForUnreadableJunitFile(
      wkspSettings,
      run,
      filteredQueue,
      junitFileUri
  ...
  try {
    ...
    throw new WkspError(
      `Unable to parse junit file ${junitFileUri.fsPath}`,
      wkspSettings.uri
    );
  }

  for (const queueItem of filteredQueue) {
    const fullFeatureName = getjUnitName(
      wkspSettings,
      queueItem.scenario.featureFileName,
      queueItem.scenario.featureFileWorkspaceRelativePath
    );
    ...
    // normal scenario
    let queueItemResults = junitContents.testsuite.testcase.filter(
      (tc) => tc.$.classname === className && tc.$.name === scenarioName
    );

    // scenario outline
    if (queueItemResults.length === 0) {
      queueItemResults = junitContents.testsuite.testcase.filter(
        (tc) =>
          tc.$.classname === className &&
          tc.$.name.substring(0, tc.$.name.lastIndexOf(" -- @")) ===
            scenarioName
      );
    }
    ...
    if (queueItemResults.length === 0 && scenarioName.includes("<")) {
      queueItemResults = junitContents.testsuite.testcase.filter((tc) => {
        const jScenName = tc.$.name.substring(
          0,
          tc.$.name.lastIndexOf(" -- @")
        );
    ...
    if (queueItemResults.length === 0) {
      throw (
        `could not match queueItem to junit result, when trying to match with $.classname="${className}", ` +
...
export function updateTestResultsForUnreadableJunitFile(
  wkspSettings: WorkspaceSettings,
  run: vscode.TestRun,
